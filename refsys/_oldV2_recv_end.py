...
class SharedData:
    def __init__(self):
        ...
        self.heartbeat_interval = 20  # 心跳包间隔(ms)
            ...
            if packet.byte2 == 0x01:  # 基础得分
                ...
                for prev_idx in range(idx-1, -1, -1):
                    ...
                    if prev_packet.byte2 == 0x04:
                        bounty_color = (prev_packet.byte3 << 8) | prev_packet.byte4
                ...
                # 找到当前包之前的所有0x03包位置
                preceding_tech_boosts = [pos for pos in tech_boost_positions if pos < idx]
                
                # 如果有技术加成包在之前，且还有剩余次数
                if preceding_tech_boosts and self.shared_data.tech_boost_count > 0:
                    # 找到最近的一个0x03包
                    last_tech_boost_idx = max(preceding_tech_boosts)
                    # 检查这个技术加成包是否还有剩余次数
                    if idx - last_tech_boost_idx <= self.shared_data.tech_boost_count:
                        has_tech_boost = True
    ...
    def update_ui(self):
        with self.shared_data.lock:
            ...
            if (self.shared_data.game_started and 
                ...
                self.shared_data.remaining_time_ms > 119500 and  # 防抖
            ...
            if (self.shared_data.game_ended or 
                ...
                self.shared_data.remaining_time_ms > 120000):
